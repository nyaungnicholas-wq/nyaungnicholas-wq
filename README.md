...
| `render_heatmap_svg.py` | 53×7 calendar SVG, one-shot diagonal reveal → `contrib-heatmap.svg` |
| `make_info_card.py` | hand-authored neofetch-style card, staggered fade-in → `info-card.svg` |

Both reveals use SMIL `<animate fill="freeze">`, not CSS `@keyframes` — GitHub does not run CSS animations in README-embedded SVGs, so a CSS reveal starting at `opacity:0` renders a permanently blank box. The checks assert SMIL and reject any hidden element without an `<animate>` to bring it back.
